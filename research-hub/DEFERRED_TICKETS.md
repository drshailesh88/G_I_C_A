...
| # | Severity | Issue | What Breaks in Production | Fix Phase | Status |
|---|----------|-------|--------------------------|-----------|--------|
| H1 | HIGH | Webhook endpoints have no signature verification | Forged delivery states | — | **FIXED** (2026-04-08) — Svix HMAC for Resend, Bearer token for Evolution API |
| H2 | HIGH | Webhook ingest swallows errors, no DLQ | Permanent delivery state loss | — | **FIXED** (2026-04-08) — Redis-backed DLQ with 7-day TTL |
| H3 | HIGH | Forward-only status uses stale read | Concurrent webhook status regression | — | **FIXED** (2026-04-08) — DB-level CASE CAS in WHERE clause |
| H4 | HIGH | `providerMessageId` not unique-constrained | Correlation ambiguity | — | **FIXED** (2026-04-08) — unique(provider, provider_message_id) on notification_log |
| H5 | HIGH | Email/WhatsApp adapters don't pass attachments | Certificates/QR silently omitted | Phase 5 | OPEN — implement R2 signed URL attachment flow |
| H6 | HIGH | Provider calls inline with no timeout/circuit breaker | Cascading saturation at scale | Phase 6 | OPEN — Inngest background jobs or AbortController timeouts |
| H7 | MEDIUM | `handleDomainEvent` only logs, doesn't send | Automation triggers do nothing | Phase 6 | OPEN — wire to sendNotification() |
