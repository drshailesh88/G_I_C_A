...
#   v2 — 2026-04-14 — Schema alignment. Renamed `event_user_roles` →
#        Clerk role + `event_user_assignments`. Behavioral intent unchanged.
...
4. Super Admin (Clerk role `org:super_admin`) bypasses tenancy isolation; every other role requires an active matching row in `event_user_assignments(event_id, auth_user_id)`. ALWAYS true.
...
7. Read-only role scope is per-event: a user with Clerk role `org:read_only` sees only events present in their `event_user_assignments` rows. ALWAYS true.
