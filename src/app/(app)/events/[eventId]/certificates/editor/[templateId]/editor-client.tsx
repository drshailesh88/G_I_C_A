...
export function CertificateEditorClient({
  ...
  const savingRef = useRef(false); // re-entrancy guard for Ctrl/Cmd+S
  ...
  // Save template to DB (re-entrancy safe: blocks overlapping saves from Ctrl/Cmd+S)
  const handleSave = useCallback(() => {
    if (!designerRef.current || status === 'archived') return;
    if (savingRef.current) return; // block concurrent saves
