...
import {
  importAccommodationBatch,
  type AccommodationImportRow,
  type AccommodationImportRowResult,
} from '@/lib/actions/accommodation';

// ── Accommodation column mapping ──────────────────────────────

const ACCOM_KNOWN_COLUMNS = [
  ...
  { key: 'hotelName', aliases: ['hotel', 'hotel name', 'property', 'accommodation', 'venue'] },
  { key: 'checkInDate', aliases: ['check in', 'check-in', 'checkin', 'arrival date', 'check in date', 'from date'] },
  { key: 'checkOutDate', aliases: ['check out', 'check-out', 'checkout', 'departure date', 'check out date', 'to date'] },
  { key: 'hotelAddress', aliases: ['address', 'hotel address', 'property address', 'location'] },
  { key: 'hotelCity', aliases: ['city', 'hotel city', 'property city', 'destination city'] },
  { key: 'roomType', aliases: ['room type', 'room', 'type of room', 'accommodation type'] },
  { key: 'roomNumber', aliases: ['room number', 'room no', 'room #'] },
  { key: 'sharedRoomGroup', aliases: ['shared room group', 'group', 'room group', 'sharing group', 'roommates'] },
  { key: 'bookingReference', aliases: ['booking ref', 'booking reference', 'confirmation', 'reservation id', 'booking id'] },
  { key: 'specialRequests', aliases: ['special requests', 'requests', 'special needs', 'preferences'] },
  { key: 'notes', aliases: ['notes', 'remarks', 'comments', 'internal notes'] },
] as const;

type AccomColumnKey = typeof ACCOM_KNOWN_COLUMNS[number]['key'];

interface AccomColumnMapping {
  csvColumn: string;
  mappedTo: AccomColumnKey | null;
  confidence: number;
}

const ACCOM_FIELD_LABELS: Record<AccomColumnKey, string> = {
  personEmail: 'Person Email',
  personPhone: 'Person Phone',
  hotelName: 'Hotel Name',
  checkInDate: 'Check-In Date (ISO)',
  checkOutDate: 'Check-Out Date (ISO)',
  hotelAddress: 'Hotel Address',
  hotelCity: 'Hotel City',
  roomType: 'Room Type',
  roomNumber: 'Room Number',
  sharedRoomGroup: 'Shared Room Group',
  bookingReference: 'Booking Reference',
  specialRequests: 'Special Requests',
  notes: 'Notes',
};

function autoMapAccomColumns(csvHeaders: string[]): AccomColumnMapping[] {
  const allAliases = ACCOM_KNOWN_COLUMNS.flatMap((col) =>
  ...
  return csvHeaders.map((header) => {
    ...
    if (exact) {
      return { csvColumn: header, mappedTo: exact.key as AccomColumnKey, confidence: 1 };
    }
    ...
    if (results.length > 0 && results[0].score !== undefined && results[0].score < 0.4) {
      return {
        csvColumn: header,
        mappedTo: results[0].item.key as AccomColumnKey,
...
const VALID_ROOM_TYPES = new Set(['single', 'double', 'twin', 'triple', 'suite', 'dormitory', 'other']);
const SPREADSHEET_FORMULA = /^[\t\r ]*[=+\-@]/;

interface ParsedAccomRow extends AccommodationImportRow {
  errors: string[];
}
...
function parseAccomRows(
  rows: Record<string, string>[],
  mappings: AccomColumnMapping[],
): ParsedAccomRow[] {
  const keyMap = new Map<AccomColumnKey, string>();
  ...
  return rows.map((row, index) => {
    ...
    const hotelName = getCellValue(row, keyMap.get('hotelName'));
    const checkInDate = getCellValue(row, keyMap.get('checkInDate'));
    const checkOutDate = getCellValue(row, keyMap.get('checkOutDate'));
    const hotelAddress = getCellValue(row, keyMap.get('hotelAddress'));
    const hotelCity = getCellValue(row, keyMap.get('hotelCity'));
    const roomType = getCellValue(row, keyMap.get('roomType'));
    const roomNumber = getCellValue(row, keyMap.get('roomNumber'));
    const sharedRoomGroup = getCellValue(row, keyMap.get('sharedRoomGroup'));
    const bookingReference = getCellValue(row, keyMap.get('bookingReference'));
    const specialRequests = getCellValue(row, keyMap.get('specialRequests'));
    const notes = getCellValue(row, keyMap.get('notes'));
    ...
    if (roomType && !VALID_ROOM_TYPES.has(roomType)) {
      errors.push(`Invalid room type "${roomType}" (must be: single, double, twin, triple, suite, dormitory, other)`);
    }

    if (checkInDate && checkOutDate && !errors.some((e) => e.includes('Check'))) {
    ...
    for (const [label, val] of [['Hotel Name', hotelName]] as [string, string][]) {
    ...
    return {
      ...
      sharedRoomGroup: sharedRoomGroup || undefined,
...
interface ImportSummary {
  ...
  skipped: number;
  errors: number;
  results: AccommodationImportRowResult[];
}
...
export function AccommodationImportClient({ eventId }: { eventId: string }) {
  const [step, setStep] = useState<Step>('upload');
  const [fileName, setFileName] = useState('');
  const [headers, setHeaders] = useState<string[]>([]);
  const [rawRows, setRawRows] = useState<Record<string, string>[]>([]);
  const [mappings, setMappings] = useState<AccomColumnMapping[]>([]);
  const [parsed, setParsed] = useState<ParsedAccomRow[]>([]);
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      setParseErrors([]);
      setMappings(autoMapAccomColumns(result.headers));
      setStep('mapping');
    };
    ...
    setMappings((prev) =>
      prev.map((m) =>
        m.csvColumn === csvColumn
          ? { ...m, mappedTo: mappedTo === '' ? null : (mappedTo as AccomColumnKey), confidence: mappedTo ? 1 : 0 }
          : m,
      ),
    );
  }
  ...
  function handlePreview() {
    setParsed(parseAccomRows(rawRows, mappings));
    setStep('preview');
  }
  ...
  function handleImport() {
    ...
    startTransition(async () => {
      try {
        const result = await importAccommodationBatch(
          eventId,
          validRows.map((r): AccommodationImportRow => ({
            ...
            sharedRoomGroup: r.sharedRoomGroup,
  ...
  const hasHotelName = mappings.some((m) => m.mappedTo === 'hotelName');
  const hasCheckIn = mappings.some((m) => m.mappedTo === 'checkInDate');
  const hasCheckOut = mappings.some((m) => m.mappedTo === 'checkOutDate');
  const requiredMapped = hasPersonMapping && hasHotelName && hasCheckIn && hasCheckOut;
  ...
  return (
    <div className="px-4 py-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-text-primary">Import Accommodation Records</h1>
          <p className="text-sm text-text-secondary">Upload a CSV file to bulk import accommodation records</p>
        </div>
      </div>
        ...
        <div className="mt-6">
          ...
          <div className="mt-4 rounded-lg border border-border bg-surface p-4">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-text-secondary sm:grid-cols-3">
              ...
              <span><span className="font-medium text-error">*</span> hotel_name</span>
              <span><span className="font-medium text-error">*</span> check_in_date (ISO)</span>
              <span><span className="font-medium text-error">*</span> check_out_date (ISO)</span>
              <span>hotel_address</span>
              <span>hotel_city</span>
              <span>room_type</span>
              <span>room_number</span>
              <span>shared_room_group</span>
              <span>booking_reference</span>
              <span>special_requests</span>
              <span>notes</span>
            </div>
            <p className="mt-2 text-xs text-text-muted">
              Room type values: single · double · twin · triple · suite · dormitory · other
              <br />
              Person must have an active travel record — rows without one will be skipped.
            </p>
          </div>
      ...
      {step === 'mapping' && (
        <div className="mt-6">
          <p className="text-sm text-text-secondary">
            Found {headers.length} columns and {rawRows.length} rows. Map each CSV column to an accommodation field.
          </p>
            ...
            {mappings.map((m) => (
              ...
              >
                ...
                >
                  ...
                  {Object.entries(ACCOM_FIELD_LABELS).map(([key, label]) => (
            ...
            <div className="mt-4 rounded-lg border border-warning/30 bg-warning/5 p-3">
              <p className="flex items-center gap-2 text-sm text-warning">
                ...
                {[
                  ...
                  !hasHotelName && 'Hotel Name',
                  !hasCheckIn && 'Check-In Date',
                  !hasCheckOut && 'Check-Out Date',
                ].filter(Boolean).join(', ')}
              </p>
            </div>
          )}
      ...
      {step === 'preview' && (
        <div className="mt-6">
                ...
                {validRows.slice(0, 5).map((r) => (
                  <div key={r.rowNumber} className="rounded-lg border border-border bg-surface p-3">
                    <p className="text-sm font-medium text-text-primary">
                      {r.hotelName}
                      {r.hotelCity && <span className="ml-2 text-xs font-normal text-text-muted">· {r.hotelCity}</span>}
                    </p>
                    <p className="mt-0.5 text-xs text-text-muted">
                      {[r.personEmail, r.personPhone, `${r.checkInDate} → ${r.checkOutDate}`].filter(Boolean).join(' | ')}
                    </p>
                    {r.sharedRoomGroup && (
                      <p className="mt-0.5 text-xs text-text-muted">Group: {r.sharedRoomGroup}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <p className="mt-4 text-xs text-text-muted">
            Note: Rows for persons without an active travel record, duplicate booking references, and unmatched persons will be reported as skipped — the batch will not fail.
          </p>
      ...
      {step === 'importing' && (
        <div className="mt-6 flex flex-col items-center py-10">
          ...
          <p className="mt-4 font-medium text-text-primary">Importing accommodation records…</p>
