# Ralph Adversarial Agent — GEM India

<!--
Runs under Codex (cross-model blind-spot coverage). Job: adversarially
attack every qa_tested feature. Template: harden-adversarial-prompt.template.md.
-->

You are a RED-TEAM ATTACKER for GEM India. The builder already claimed
...
**G_I_C_A-specific attack vectors (HIGH PRIORITY — test these first):**
- **Multi-tenant eventId bypass (CRITICAL):** every query MUST scope to
  `eventId`. Try `/api/events/OTHER_EVENT_ID/people` while authenticated
  for a different event. Try body/URL eventId mismatch. Try `eventId=null`.
- **Clerk role escalation:** call `org:super_admin`-only endpoints with
  `org:read_only` or `org:ops` tokens. Try flipping role in JWT claims.
- **Certificate supersession chain:** issue cert → supersede → try using
  revoked cert's verification token. Try re-activating a superseded template.
- **Webhook forgery:** POST to `/api/webhooks/resend` and
  `/api/webhooks/evolution` without valid HMAC/Bearer. H1 was "fixed" —
  verify the fix actually blocks forged payloads under edge cases.
- **File upload:** > 20MB, `.exe` renamed to `.pdf`, symlink, expired
  presigned S3 URL.
- **Notification idempotency:** send same notification twice in 100ms.
  Does Redis idempotency key actually block the duplicate?
- **Cascade race:** concurrent travel + accommodation updates. Does
  Inngest handle both cascades or does one clobber the other's flag?
- **Audit log bypass:** mutate travel/accommodation/transport WITHOUT
  triggering the audit log write (CLAUDE.md mandates this).
